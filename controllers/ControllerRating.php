...
class ControllerRating
{
    ...
    public function checkUserCanRate($store_id, $user_id) {
        $stmt = $this->pdo->prepare('SELECT * FROM tbl_storefinder_ratings WHERE store_id = :store_id AND user_id = :user_id');
        $stmt->execute( array('store_id' => $store_id, 'user_id' => $user_id) );
        foreach ($stmt as $row) {
            $itm = new Rating();
    ...
    public function getRatingByStoreId($store_id) {
        $stmt = $this->pdo->prepare('SELECT * FROM tbl_storefinder_ratings WHERE store_id = :store_id');
    ...
    public function getRatingCountByStoreId($store_id) {
        $stmt = $this->pdo->prepare('SELECT * FROM tbl_storefinder_ratings WHERE store_id = :store_id');
        $stmt->execute( array('store_id' => $store_id) );
        return $stmt->rowCount();
    }

    public function getRatingTotalByStoreId($store_id) {
        $stmt = $this->pdo->prepare('SELECT * FROM tbl_storefinder_ratings WHERE store_id = :store_id');
        $stmt->execute( array('store_id' => $store_id) );
        $count = 0;
        $total = 0;
