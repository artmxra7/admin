...
class ControllerNews
{
    ...
    public function deleteNews($news_id, $is_deleted) {
        $stmt = $this->pdo->prepare('UPDATE tbl_storefinder_news SET is_deleted = :is_deleted WHERE news_id = :news_id ');
        $result = $stmt->execute( array('news_id' => $news_id, 'is_deleted' => $is_deleted) );
        return $result ? true : false;
    }
    ...
    public function getNewsBySearching($search) {
        $stmt = $this->pdo->prepare('SELECT * FROM tbl_storefinder_news WHERE is_deleted = 0 AND news_title LIKE :search ORDER BY news_id DESC');
    ...
    public function getNewsByNewsId($news_id) {
        $stmt = $this->pdo->prepare('SELECT * FROM tbl_storefinder_news WHERE news_id = :news_id');
    ...
    public function getNewsAtRange($begin, $end) {
        $stmt = $this->pdo->prepare('SELECT * FROM tbl_storefinder_news WHERE is_deleted = 0 ORDER BY created_at DESC LIMIT :beg, :end');
    ...
    public function formatNews($row) {
        $itm = new News();
    ...
    public function getNewsLatestAtCount($max) {
        $stmt = $this->pdo->prepare('SELECT * FROM tbl_storefinder_news WHERE is_deleted = 0 ORDER BY created_at DESC LIMIT 0, :max');
