...
<?php 
    ...
    if( isset($_POST['submit']) ) {
      ...
      if( $_FILES["file_category"]["size"] > 0  )
        $itm->category_icon = $extras->uploadFile("file_category", "category_");

      $itm->map_icon = $_POST['map_icon'];
      if( $_FILES["file_map_category"]["size"] > 0  )
        $itm->map_icon = $extras->uploadFile("file_map_category", "category_map_");
            ...
            <div class="col-lg-8">
              ...
              <div class="card shadow mb-4">
                <div class="card-header py-3">
                  <h6 class="m-0 font-weight-bold text-primary"><?php echo __EDIT_CATEGORY; ?></h6>
                </div>
                <div class="card-body">
                  <form class="user" action="" method="POST" enctype="multipart/form-data">
                    <div class="form-group">
                      <label><?php echo __CATEGORY_NAME; ?></label>
                      <input type="text" class="form-control " placeholder="<?php echo __CATEGORY_NAME; ?>" name="category" required value="<?php echo $category->category; ?>">
                    </div>

                    <div class="form-group">
                      <label><?php echo __CATEGORY_ICON_URL; ?></label>
                      <input type="text" class="form-control " placeholder="<?php echo __CATEGORY_ICON_URL; ?>" name="category_icon" value="<?php echo $category->category_icon; ?>">
                    </div>

                    <div class="form-group">
                      <label><?php echo __CATEGORY_ICON_UPLOAD; ?></label>
                      <br/>
                      <input type="file" name="file_category" accept="image/*"/>
                    </div>

                    <div class="form-group">
                      <label><?php echo __CATEGORY_MAP_ICON_URL; ?></label>
                      <input type="text" class="form-control " placeholder="<?php echo __CATEGORY_MAP_ICON_URL; ?>" name="map_icon" value="<?php echo $category->map_icon; ?>">
                    </div>

                    <div class="form-group">
                      <label><?php echo __CATEGORY_MAP_ICON_UPLOAD; ?></label>
                      <br/>
                      <input type="file" name="file_map_category" accept="image/*"/>
                    </div>

                    <div class="form-group">
                      <label><?php echo __PARENT_CATEGORY; ?></label>
                      <select class="form-control" name="pid">
                        <option value="0"><?php echo __MAIN_CATEGORY; ?></option>
                        <?php echo $extras->toUL1($categories1, 0, $category->pid, $category_id); ?>
                      </select>
                    </div>

                    <button type="submit" name="submit" class="btn btn-primary "><?php echo __SAVE; ?></button>
                    <a href="categories.php" class="btn btn-danger "><?php echo __CANCEL; ?></a>
                  </form>
