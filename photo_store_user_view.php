...
<?php 
    ...
    if( !empty($params['photo_id']) && !empty($params['is_deleted']) ) {
        $controller->deletePhoto($params['photo_id'], 1);
    }
        ...
        <div class="container-fluid">
          <div class="card shadow mb-4">
            <div class="card-header py-3">
              <h6 class='m-0 font-weight-bold text-primary'><a href="<?php echo $url; ?>" style="margin-right:10px;"><i class="fas fa-arrow-left"></i></a><a href="photo_store_user_insert.php" style="float:right;text-decoration:none;"><?php echo __ADD_PHOTO; ?> <i class="fas fa-plus"></i></a></h6>
            </div>
          </div>
            ...
            <?php 
              if($photos_store != null) {
                ...
                foreach ($photos_store as $photo_store)  {
                  ...
                  $params = array();
                  $params[0] = array( 'photo_id', $photo_store->photo_id );
                  $updateUrl = $extras->encryptParams(KEY_SALT, $params, 'photo_store_user_update.php');

                  $params = array();
                  $params[0] = array( 'photo_id', $photo_store->photo_id );
                  $params[1] = array( 'store_id', $photo_store->store_id );
                  $params[2] = array( 'is_deleted', "1" );
                  $deleteUrl = $extras->encryptParams(KEY_SALT, $params, 'photo_store_user_view.php');
